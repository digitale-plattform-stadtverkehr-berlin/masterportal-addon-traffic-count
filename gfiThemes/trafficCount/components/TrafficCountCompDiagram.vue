...
export default {
    ...
    methods: {
        ...
        interpolateData (odata, medianSurround = 5) {
          let mdata = odata.slice(); // first copy old array

          for (let i=0, len=odata.length|0; i<len; i=i+1|0) {
            if ((i >=1) && (i < (odata.length - 1))) { // do not compute first or last element
              let leftSurround = Math.max(0, (i-medianSurround));
              let rightSurround = Math.min((i+medianSurround), (odata.length-1));
              let surrounding = odata.slice(leftSurround, rightSurround);
              mdata[i] = this.calcMedian(surrounding);
            }
          }
          // cleanup
        ...
        createDataForDiagram (apiData, colors, callbackRenderLabelLegend) {
            ...
            apiData.forEach((dataObj, idx) => {
                ...
                const origData = Object.values(dataObj[meansOfTransportKey]);
                let useColorIdx = apiData.length - (idx + 1);
                ...
                if (this.doInterpolate) {
                    ...
                    datasets.push({
                      label: datetimes.length > 0 && typeof callbackRenderLabelLegend === "function" ? callbackRenderLabelLegend(datetimes[0])+"("+this.$t("additional:modules.tools.gfi.themes.trafficCount.interpolated")+")" : "",
