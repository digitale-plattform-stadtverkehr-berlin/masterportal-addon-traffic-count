...
export class TrafficCountCache {
     ...
     * @returns {Void}  -
     */
    updateDescription (thingId, onupdate, onerror, onstart, oncomplete, simpleCacheCallOpt = null, trafficCountApiOpt = null) {
        const key = "updateDescription" + thingId;

        (simpleCacheCallOpt || this.simpleCacheCall).bind(this)(key, (callback) => {
            (trafficCountApiOpt || this.api).updateDescription(thingId, (...args) => {
                if (typeof callback === "function") {
                    return callback(...args);
                }
                return null;
     ...
     * @returns {Void}  -
     */
    updateProperty (thingId, propertyName, onupdate, onerror, onstart, oncomplete, simpleCacheCallOpt = null, trafficCountApiOpt = null) {
        const key = "updateProperty" + thingId + propertyName;

        (simpleCacheCallOpt || this.simpleCacheCall).bind(this)(key, (callback) => {
            (trafficCountApiOpt || this.api).updateProperty(thingId, propertyName, (...args) => {
                if (typeof callback === "function") {
                    return callback(...args);
                }
                return null;
     ...
     * @returns {Void}  -
     */
    updateDay (thingId, meansOfTransport, dayInterval, day, onupdate, onerror, onstart, oncomplete, simpleCacheCallOpt = null, trafficCountApiOpt = null) {
        const key = "updateDay" + thingId + meansOfTransport + dayInterval + day;

        (simpleCacheCallOpt || this.simpleCacheCall).bind(this)(key, (callback) => {
            (trafficCountApiOpt || this.api).updateDay(thingId, meansOfTransport, dayInterval, day, (...args) => {
                if (typeof callback === "function") {
                    return callback(...args);
                }
                return null;
     ...
     * @returns {Void}  -
     */
    updateDaySpeedMedium (thingId, meansOfTransportSpeed, meansOfTransportCount, dayInterval, day, onupdate, onerror, simpleCacheCallOpt = null, trafficCountApiOpt = null) {
        const key = "updateDaySpeedMedium" + thingId + meansOfTransportSpeed + meansOfTransportCount + dayInterval + day;

        (simpleCacheCallOpt || this.simpleCacheCall).bind(this)(key, (callback) => {
            (trafficCountApiOpt || this.api).updateDaySpeedMedium(thingId, meansOfTransportSpeed, meansOfTransportCount, dayInterval, day, (...args) => {
                if (typeof callback === "function") {
                    return callback(...args);
                }
                return null;
     ...
     * @returns {Void}  -
     */
    updateYear (thingId, meansOfTransport, dayInterval, year, onupdate, onerror, onstart, oncomplete, simpleCacheCallOpt = null, trafficCountApiOpt = null) {
        const key = "updateYear" + thingId + meansOfTransport + dayInterval + year;

        (simpleCacheCallOpt || this.simpleCacheCall).bind(this)(key, (callback) => {
            (trafficCountApiOpt || this.api).updateYear(thingId, meansOfTransport, dayInterval, year, (...args) => {
                if (typeof callback === "function") {
                    return callback(...args);
                }
                return null;
     ...
     * @returns {Void}  -
     */
    updateTotal (thingId, meansOfTransport, dayInterval, onupdate, onerror, onstart, oncomplete, simpleCacheCallOpt = null, trafficCountApiOpt = null) {
        const key = "updateTotal" + thingId + meansOfTransport + dayInterval;

        (simpleCacheCallOpt || this.simpleCacheCall).bind(this)(key, callback => {
            (trafficCountApiOpt || this.api).updateTotal(thingId, meansOfTransport, dayInterval, (...args) => {
                if (typeof callback === "function") {
                    return callback(...args);
                }
                return null;
     ...
     * @returns {Void}  -
     */
    subscribeLastUpdate (thingId, meansOfTransport, dayInterval, onupdate, onerror, onstart, oncomplete, simpleCacheCallOpt = null, trafficCountApiOpt = null) {
        const key = "subscribeLastUpdate" + thingId + meansOfTransport + dayInterval;

        (simpleCacheCallOpt || this.simpleCacheCall).bind(this)(key, (callback) => {
            (trafficCountApiOpt || this.api).subscribeLastUpdate(thingId, meansOfTransport, dayInterval, (...args) => {
                if (typeof callback === "function") {
                    return callback(...args);
                }
                return null;
     ...
     * @returns {Void}  -
     */
    getUnitOfMeasurement (thingId, meansOfTransport, interval, onsuccess, onerror, onstart, oncomplete, simpleCacheCallOpt = null, trafficCountApiOpt = null) {
        const key = "getUnitOfMeasurement" + thingId + interval + meansOfTransport;

        (simpleCacheCallOpt || this.simpleCacheCall).bind(this)(key, callback => {
            (trafficCountApiOpt || this.api).getUnitOfMeasurement(thingId, meansOfTransport, interval, (...args) => {
                if (typeof callback === "function") {
                    return callback(...args);
                }
                return null;
